...
<section id="section-pengguna-form" class="content-section block font-sans">
        <div class="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p class="text-slate-500 text-sm">Daftarkan akun administrator baru untuk pengelolaan portal.</p>
          </div>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 max-w-2xl">
          <form action="{{ route('admin.store_user') }}{{ \App\Support\AdminNavigation::context() ? '?'.http_build_query(\App\Support\AdminNavigation::context()) : '' }}" method="POST" class="space-y-6">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              <button type="submit" class="px-6 py-2.5 rounded-xl font-semibold bg-blue-600 text-white hover:bg-blue-700 shadow-sm transition-all text-sm focus:outline-none focus:ring-2 focus:ring-blue-600">Daftarkan Pegawai</button>
            </div>
          </form>
        </div>
      </section>
