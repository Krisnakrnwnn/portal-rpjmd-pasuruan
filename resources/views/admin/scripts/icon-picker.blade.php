<script>
  ...
  function selectIcon(type, svg, name) {
    ...
    document.getElementById(`sector-icon-${type}-value`).dispatchEvent(new Event('change', {bubbles:true}));
  ...
  function initIconPickers() {
    ['add', 'edit'].forEach(type => {
      ...
      HERO_ICONS.forEach(icon => {
        ...
        btn.type = 'button';
        btn.className = 'p-3 flex items-center justify-center rounded-xl hover:bg-slate-100 hover:text-slate-700 border border-gray-100 transition-all';
...
document.addEventListener('DOMContentLoaded', () => {
 const field = document.querySelector('input[name="icon"]');
 if (!field?.value) return;
 const type = field.id.includes('-edit-') ? 'edit' : 'add';
 // Only render icons from the existing picker allowlist. The hidden stored value is preserved.
 const known = HERO_ICONS.find(icon => icon.svg === field.value);
 if (known) selectIcon(type, known.svg, known.name);
});
