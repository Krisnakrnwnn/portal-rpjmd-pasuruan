...
<head>
  ...
  <title>@yield('title', 'Layanan Informasi RPJMD Kota Pasuruan')</title>
  <meta name="description" content="@yield('meta_description', 'Portal Layanan Informasi RPJMD Kota Pasuruan. Transparansi data, perencanaan, dan capaian pembangunan kota.')" />
  
  <!-- Open Graph / Facebook -->
  <meta property="og:type" content="website">
  <meta property="og:url" content="{{ url()->current() }}">
  <meta property="og:title" content="@yield('title', 'Layanan Informasi RPJMD Kota Pasuruan')">
  <meta property="og:description" content="@yield('meta_description', 'Portal Layanan Informasi RPJMD Kota Pasuruan. Transparansi data, perencanaan, dan capaian pembangunan kota.')">
  <meta property="og:image" content="@yield('og_image', asset('hero.png'))">

  <!-- Twitter -->
  <meta property="twitter:card" content="summary_large_image">
  <meta property="twitter:url" content="{{ url()->current() }}">
  <meta property="twitter:title" content="@yield('title', 'Layanan Informasi RPJMD Kota Pasuruan')">
  <meta property="twitter:description" content="@yield('meta_description', 'Portal Layanan Informasi RPJMD Kota Pasuruan. Transparansi data, perencanaan, dan capaian pembangunan kota.')">
  ...
  <div id="page-preloader">
    <div class="relative flex items-center justify-center">
      ...
      <div class="preloader-logo">
        <span style="color:white; font-weight:900; font-size:28px; font-family:'Outfit',sans-serif;">R</span>
      </div>
    </div>
    <div>
        ...
        <div class="flex items-center gap-3">
          <a href="{{ route('home') }}" class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Kota Pasuruan</div>
            </div>
          </a>
        </div>
  ...
  <footer class="bg-[#041a42] text-gray-300 pt-16 pb-8 border-t border-blue-900 w-full mt-auto relative z-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
        <div class="col-span-1 md:col-span-2">
          <div class="flex items-center gap-3 mb-6">
            <div class="w-8 h-8 rounded bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center shadow-md">
              <span class="text-white font-black text-sm">R</span>
            </div>
            <div class="font-black text-lg tracking-tight text-white leading-none">RPJMD Kota Pasuruan</div>
          </div>
          <p class="text-gray-400 text-sm leading-relaxed max-w-sm mb-6 font-light">Portal Layanan Informasi Rencana Pembangunan Jangka Menengah Daerah (RPJMD) Kota Pasuruan — menyajikan data perencanaan, capaian kinerja, dan informasi pembangunan kota secara transparan untuk masyarakat.</p>
        ...
        <div>
          ...
          <ul class="space-y-4 text-sm text-gray-400 flex flex-col">
            <li class="flex items-start group">
              <a href="{{ route('kontak') }}" class="hover:text-yellow-400 transition-colors">Jl. Pahlawan No. 44, Kota Pasuruan, Jawa Timur 67114</a>
            </li>
            <li class="flex items-center group">
              <a href="mailto:rpjmd@pasuruankota.go.id" class="hover:text-yellow-400 transition-colors">rpjmd@pasuruankota.go.id</a>
            </li>
          </ul>
        </div>
      </div>
      <div class="pt-8 border-t border-blue-900/50 flex flex-col md:flex-row justify-between items-center auto-cols-auto gap-4 text-sm text-gray-500 text-center md:text-left transition-all">
        <p>&copy; {{ date('Y') }} RPJMD Kota Pasuruan. Hak Cipta Dilindungi.</p>
        <div class="flex gap-4">
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const chatbotHTML = `
      <div id="ai-chatbot-widget" class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        ...
        <div id="chat-window" class="hidden w-80 sm:w-96 md:w-[400px] bg-white rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] border border-gray-100 mb-4 overflow-hidden flex-col h-[500px] transform transition-all origin-bottom-right">
          ...
          <div id="chat-messages" class="flex-1 p-5 bg-[#F8FAFC] overflow-y-auto flex flex-col gap-4" style="background-image: url('https://www.transparenttextures.com/patterns/absurdity.png')">
            ...
            <!-- Quick Options -->
            <div id="chat-quick-options" class="flex flex-wrap gap-2 ml-10 -mt-2 animate-chat-msg opacity-0" style="animation-delay: 0.3s forwards">
              <button class="quick-btn text-[11px] font-bold bg-white border border-blue-200 text-blue-600 hover:bg-blue-60 px-3 py-1.5 rounded-full transition-colors shadow-sm hover:shadow" data-text="Apa Visi dan Misi Kota Pasuruan?">Visi Misi Kota</button>
              <button class="quick-btn text-[11px] font-bold bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-full transition-colors shadow-sm hover:shadow" data-text="Berapa jumlah Program Prioritas saat ini?">Program Prioritas</button>
              <button class="quick-btn text-[11px] font-bold bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-full transition-colors shadow-sm hover:shadow" data-text="Tolong jelaskan apa itu RPJMD secara singkat.">Apa itu RPJMD?</button>
            </div>
          ...
          <div class="p-4 bg-white border-t border-gray-100 flex items-center gap-3 shrink-0">
            ...
            <input id="chat-input" type="text" placeholder="Tanyakan seputar RPJMD Kota Pasuruan..." class="flex-1 bg-gray-50 border border-gray-200 rounded-full px-5 py-2.5 text-sm focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 transition-all font-medium">
            <button id="chat-send" class="w-11 h-11 rounded-full bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 shadow-md hover:shadow-lg transition-transform hover:-translate-y-0.5 shrink-0 focus:outline-none">
        ...
        <button id="chat-toggle" class="group relative w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-800 rounded-full shadow-[0_10px_25px_rgba(37,99,235,0.5)] border-4 border-white/20 flex items-center justify-center text-white hover:scale-110 transition-all duration-300 focus:outline-none cursor-pointer mt-4">
          ...
          <span id="chat-notif-dot" class="absolute top-0 right-0 w-4 h-4 bg-red-500 border-2 border-white rounded-full flex items-center justify-center animate-bounce"></span>
        </button>
    ...
    function sendMessage() {
      ...
      const quickResponses = {
        "Apa Visi dan Misi Kota Pasuruan?": "Pemerintah Kota Pasuruan memiliki **Visi**:<br>\"Mewujudkan Kota Pasuruan Kota Madinah (Maju Ekonominya, Indah Kotanya, Harmoni Warganya).\"<br><br>**Misi Utama**:<br>1. Mempercepat pertumbuhan ekonomi<br>2. Meningkatkan tata kelola pemerintahan yang baik<br>3. Pemerataan pembangunan infrastruktur<br>4. Membangun SDM unggul dan berdaya saing.",
        "Berapa jumlah Program Prioritas saat ini?": "Berdasarkan data RPJMD terbaru, Pemerintah Kota Pasuruan saat ini menargetkan lebih dari **78+ Program Prioritas** yang dieksekusi secara berkesinambungan di berbagai sektor (Infrastruktur, Pelayanan Publik, Pendidikan, dll) selama periode 5 tahun ke depan.",
