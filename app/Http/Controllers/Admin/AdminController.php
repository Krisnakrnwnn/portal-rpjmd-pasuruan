...
use App\Http\Requests\Admin\TestAiModelRequest;
...
class AdminController extends Controller
{
    ...
    public function storeDocumentCategory(Request $request)
    {
        ...
        return redirect(AdminNavigation::url('admin.dokumen.index'))->with('success', 'Kategori dokumen berhasil ditambahkan!');
    }
    ...
    public function updateDocumentCategory(Request $request, $id)
    {
        ...
        return redirect(AdminNavigation::url('admin.dokumen.index'))->with('success', 'Kategori dokumen berhasil diperbarui!');
    }
    ...
    public function destroyDocumentCategory($id)
    {
        ...
        if ($category->documents()->count() > 0) {
            return redirect(AdminNavigation::url('admin.dokumen.index'))->with('error', 'Kategori ini tidak dapat dihapus karena masih digunakan oleh dokumen!');
        }
    ...
    {
        $this->validateStatKeys($request, 'stats');
        foreach ($request->stats as $key => $value) {
            Stat::where('key', $key)->update(['value' => $value]);
        }
        Activity::log('Statistik', 'Update', 'Memperbarui statistik capaian RPJMD.');

        return redirect(AdminNavigation::url('admin.dashboard'))->with('success', 'Statistik capaian RPJMD berhasil diperbarui!');
    }

    public function updateHeroStats(Request $request)
    {
        $this->validateStatKeys($request, 'hero_stats', true);
        foreach ($request->hero_stats as $key => $value) {
            Stat::where('key', $key)->update(['value' => $value]);
        }
        Activity::log('Statistik', 'Update', 'Memperbarui statistik utama halaman beranda.');

        return redirect(AdminNavigation::url('admin.dashboard'))->with('success', 'Statistik utama beranda berhasil diperbarui!');
    }
    ...
    public function storeHeroStat(Request $request)
    {
        ...
        $suffix = '';
        $attempt = 0;
        while (Stat::where('key', $key.$suffix)->exists()) {
            $attempt++;
            $suffix = '_'.$attempt;
        }
        $key .= $suffix;
        ...
        return redirect(AdminNavigation::url('admin.dashboard'))->with('success', 'Statistik beranda "'.$request->label.'" berhasil ditambahkan!');
    }
    ...
    public function deleteHeroStat($id)
    {
        ...
        if (! str_starts_with($stat->key, 'hero_')) {
            return redirect(AdminNavigation::url('admin.dashboard'))->with('error', 'Hanya statistik beranda yang dapat dihapus di sini.');
        }

        Activity::log('Statistik', 'Hapus', 'Menghapus statistik beranda: '.$stat->label);
        $stat->delete();

        return redirect(AdminNavigation::url('admin.dashboard'))->with('success', 'Statistik "'.$stat->label.'" berhasil dihapus!');
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        Log::info('updateProfile Request inputs: ', $request->all());
        ...
        foreach ($profiles as $key => $content) {
            ...
            Log::info("Berhasil memproses key: {$key}");
        }
    ...
    public function storeUser(Request $request)
    {
        ...
        return redirect(AdminNavigation::url('admin.pengguna.index'))->with('success', 'Pengguna baru berhasil ditambahkan!');
    }
    ...
    public function updateUser(Request $request, $id)
    {
        ...
        return redirect(AdminNavigation::url('admin.pengguna.index'))->with('success', 'Data pengguna berhasil diperbarui!');
    }
    ...
    public function deleteUser($id)
    {
        ...
        if ($user->id === auth()->id()) {
            return redirect(AdminNavigation::url('admin.pengguna.index'))
        ...
        return redirect(AdminNavigation::url('admin.pengguna.index'))->with('success', 'Pengguna berhasil dihapus!');
    }
    ...
    public function storeDocument(Request $request)
    {
        ...
        $cat = DocumentCategory::find($request->document_category_id);
        ...
        return redirect(AdminNavigation::url('admin.dokumen.index'))->with('success', "$uploadedCount Dokumen berhasil ditambahkan!");
    }
    ...
    public function updateDocument(Request $request, $id)
    {
        ...
        $cat = DocumentCategory::find($data['document_category_id']);
        ...
        return redirect(AdminNavigation::url('admin.dokumen.index'))->with('success', 'Dokumen berhasil diperbarui!');
    }
    ...
    public function deleteDocument($id)
    {
        ...
        return redirect(AdminNavigation::url('admin.dokumen.index'))->with('success', 'Dokumen berhasil dihapus!');
    }

    private function validateStatKeys(Request $request, string $field, bool $heroOnly = false): void
    {
        $request->validate([$field => ['required', 'array'], "$field.*" => ['required', function ($attribute, $value, $fail) {
            if (! is_scalar($value)) {
                $fail('Nilai statistik harus berupa teks atau angka.');
            }
        }]]);
        $keys = array_keys($request->input($field));
        // Resolve database-equivalent keys too (e.g. accent-insensitive collations).
        $matchedKeys = Stat::whereIn('key', $keys)->pluck('key')->all();
        foreach (array_merge($keys, $matchedKeys) as $key) {
            // Case-insensitive guard also covers case-insensitive database collations.
            $normalized = strtolower(rtrim((string) $key));
            if (in_array($normalized, AiSettings::RESERVED_KEYS, true) || ($heroOnly && ! str_starts_with($normalized, 'hero_'))) {
                throw ValidationException::withMessages([$field => 'Pengaturan AI hanya dapat diubah melalui Setelan Model AI.']);
            }
        }
    }
    ...
    public function updateSettings(UpdateAiSettingsRequest $request, AiSettings $settings)
    {
        ...
        return redirect(AdminNavigation::url('admin.setelan.index'))->with('success', 'Setelan model AI berhasil diperbarui!');
    }
