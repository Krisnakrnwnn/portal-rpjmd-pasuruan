    ...
    public function chat(Request $request, AIManager $ai)
    {
        ...
        try {
            ...
            while ($retryCount < $maxRetries) {
                $embedResponse = Http::timeout(30)->withHeaders(['x-goog-api-key' => $apiKey])
                ...
                if ($embedResponse->status() === 429) {
                    $retryCount++;
                    Sleep::for(2)->seconds(); // Preserve the existing retry delay.
                } else {
                    break; // Other error
                }
            }
            ...
            $status = $e->status;
            \Illuminate\Support\Facades\Log::warning('Chatbot generation failed', ['category' => $e->category, 'status' => $status]);
            ...
            \Illuminate\Support\Facades\Log::warning('Chatbot request failed', ['category' => 'internal']);

            return response()->json(['reply' => 'Sistem sedang sibuk atau terjadi kesalahan internal. Silakan coba lagi nanti.'], 500);
        }
    }
